define(["require", "exports", "./Event", "localforage"], function (require, exports, Event, localforage_1) {
    "use strict";
    //some settings, which I will put somewhere sometime
    const CAL_CACHE_DAYS = 5; //days of calendar to cache ahead of time
    const EVENT_CACHE_KEY = '0'; //arbitrary cache keys, b/c I made my own key system. this cache key is for the array of parsed events
    const SYNC_CACHE_KEY = '1'; //this key is for a sync token from the calendar API 
    const LASTDAY_CACHE_KEY = '2'; //this key stores the last day the API cached, so as to know when to refresh it's sync and cache stuff
    class CalendarData {
        constructor(http) {
            //init http
            this.http = http;
        ...
        getNewCalendar() {
            ...
            localforage_1.default.setItem(LASTDAY_CACHE_KEY, sometime.toISOString());
            this.lastStored = sometime;
            return this.http.getCalendar({ timeMin: today.toISOString(), timeMax: sometime.toISOString() }).then((data) => {
                //store sync token for later use
                this.nextSyncToken = data.nextSyncToken;
                localforage_1.default.setItem(SYNC_CACHE_KEY, data.nextSyncToken);
                //sort events by ID
                //by sort I mean create a dict and let javascript do the heavy lifting
                let evList = {};
                let n = data.items.length;
                for (let i = 0; i < n; i++) {
                    let betterEvent = Event.fromGCAL(data.items[i]);
                    //store event
                ...
                localforage_1.default.setItem(EVENT_CACHE_KEY, JSON.stringify(evList));
        ...
        dayCheckFunc() {
            ...
            if (this.lastStored != undefined) {
                ...
                if (this.lastStored.getTime() > new Date().setHours(0, 0, 0, 0))
                    return Promise.resolve();
                else
                    return Promise.reject("Cached date out of date");
            }
            //check if stored calendar has today
            return localforage_1.default.getItem(LASTDAY_CACHE_KEY).then((lastDate) => {
                ...
                if (date.getTime() < new Date().setHours(0, 0, 0, 0))
                    return Promise.reject("Cache out of date");
        ...
        cacheLoadCheckFunc() {
            if (this.eventList != undefined)
                return Promise.resolve();
            //load it from cache
            return localforage_1.default.getItem(EVENT_CACHE_KEY).then((events) => {
                if (events == null)
        ...
        syncTokenCheckFunc() {
            ...
            return localforage_1.default.getItem(SYNC_CACHE_KEY).then((token) => {
                if (token == null)
                    return Promise.reject("No stored sync token");
                //sir, we got a token
        ...
        syncFunc() {
            return this.http.getCalendar({ syncToken: this.nextSyncToken }).then((syncData) => {
                //refresh syncToken
                this.nextSyncToken = syncData.nextSyncToken;
                localforage_1.default.setItem(SYNC_CACHE_KEY, syncData.nextSyncToken);
                ...
                //and finnally cache dat shizzle
                localforage_1.default.setItem(EVENT_CACHE_KEY, JSON.stringify(this.eventList));
                return;
            }).catch((err) => {
        ...
        initCalendarCache() {
            ...
            this.initPromise = localforage_1.default.ready().then(() => {
        ...
        syncCalendar() {
            ...
            return Promise.all([this.dayCheckFunc(), this.syncTokenCheckFunc()]).then(this.syncFunc, this.getNewCalendar);
        }
        ...
        getTodaysEvents() {
            ...
            for (let event in this.eventList) {
                ...
                if (this.eventList.hasOwnProperty(event)) {
                    ...
                    let betterEvent = Event.fromCachable(this.eventList[event]);
                    if (betterEvent.getTime().toDateString() == new Date().toDateString())
                        retEvent.push(betterEvent);
                }
            }
            return retEvent;
        }
        clearCache() { localforage_1.default.clear(); }
    }
    return CalendarData;
});
